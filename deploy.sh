...
AWS_ECS_SERVICE_GEN_CONSUMERS=$(eval "echo \$${ENV}_AWS_ECS_SERVICE_GEN_CONSUMERS")

KAFKA_CLIENT_CERT=$(eval "echo \$${ENV}_KAFKA_CLIENT_CERT")
KAFKA_CLIENT_CERT_KEY=$(eval "echo \$${ENV}_KAFKA_CLIENT_CERT_KEY")
KAFKA_GROUP_ID=$(eval "echo \$${ENV}_KAFKA_GROUP_ID")
KAFKA_URL=$(eval "echo \$${ENV}_KAFKA_URL")
KAFKA_CONSUMER_TOPICS=$(eval "echo \$${ENV}_KAFKA_CONSUMER_TOPICS")
...
TC_API_V6_BASE_URL=$(eval "echo \$${ENV}_TC_API_V6_BASE_URL")
...
make_task_def(){
	task_template='[
  {
	...
	task_def=$(printf "$task_template" $1 $AWS_ACCOUNT_ID $AWS_REGION $AWS_REPOSITORY $TAG $2 $3 $4 $ENV "$KAFKA_CLIENT_CERT" "$KAFKA_CLIENT_CERT_KEY" $KAFKA_GROUP_ID $KAFKA_URL $DATABASE_URL $AUTHSECRET $TC_API_BASE_URL $TC_API_V3_BASE_URL $TC_API_V4_BASE_URL $TC_API_V5_BASE_URL $TC_API_V6_BASE_URL $MESSAGE_API_BASE_URL $CONNECT_URL $ENABLE_EMAILS $MENTION_EMAIL $REPLY_EMAIL_PREFIX $REPLY_EMAIL_DOMAIN $REPLY_EMAIL_FROM $DEFAULT_REPLY_EMAIL $ENABLE_DEV_MODE $DEV_MODE_EMAIL $LOG_LEVEL $VALID_ISSUERS $PORT "$API_CONTEXT_PATH" "$AUTH0_URL" "$AUTH0_AUDIENCE" $AUTH0_CLIENT_ID "$AUTH0_CLIENT_SECRET" $TOKEN_CACHE_TIME "$AUTH0_PROXY_SERVER_URL" $KAFKA_CONSUMER_TOPICS $AWS_ECS_CLUSTER $AWS_REGION $AWS_ECS_CLUSTER $ENV)
}
...
deploy_cluster $AWS_ECS_SERVICE_GEN_CONSUMERS "npm" "run" "startConsumer"

check_service_status $AWS_ECS_SERVICE_API

check_service_status $AWS_ECS_SERVICE_CONSUMERS

check_service_status $AWS_ECS_SERVICE_GEN_CONSUMERS
